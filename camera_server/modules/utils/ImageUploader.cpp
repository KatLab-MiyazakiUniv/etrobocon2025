                                ...
                                int maxAttempts)
{
  ...
  while(attempts < maxAttempts) {
    std::string command = "make -f Makefile.upload upload-image FILE_PATH=" + absolutePath;
                                       ...
                                       const std::string& uploadFileName, int maxAttempts)
{
  ...
  while(attempts < maxAttempts) {
    ...
    std::string command = "make -f Makefile.upload upload-minifig-image FILE_PATH=" + absolutePath;
    int result = CommandExecutor::exec(command);
    if(result == 0) {
      return true;
    }
