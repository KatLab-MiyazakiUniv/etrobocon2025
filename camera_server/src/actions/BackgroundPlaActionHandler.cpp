...
#include "utils/FrameSave.h"
#include "utils/ImageUploader.h"
#include <thread>
#include <iostream>
...
BackgroundPlaActionHandler::BackgroundPlaActionHandler(CameraCapture& _camera)
  : camera(_camera) {
    firstAttemptResult.wasDetected = false;
}

void BackgroundPlaActionHandler::runPlaCameraAction(const CameraServer::BackgroundPlaActionRequest& request) {
...
void BackgroundPlaActionHandler::execute(const CameraServer::BackgroundPlaActionRequest& request, CameraServer::BackgroundPlaActionResponse& response) {
    cv::Mat frame;
    ...
    if (request.position == 0) {
        ...
        if (this->firstAttemptResult.wasDetected && this->firstAttemptResult.direction == BackgroundDirection::FRONT) {
        ...
        cout << "Subsequent attempt (position " << request.position << ") after failed detection." << endl;
        string fileName = "bestframe_" + to_string(request.position) + ".jpg";
        FrameSave::save(frame, filePath, fileName);
