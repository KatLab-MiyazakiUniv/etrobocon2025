...
MiniFigActionHandler::MiniFigActionHandler(CameraCapture& _camera)
  : camera(_camera) {
...
void MiniFigActionHandler::detectDirection(cv::Mat& frame, MiniFigDirectionResult& result)
{
    ...
    switch(result.direction) {
        case MiniFigDirection::FRONT: cout << "Direction: FRONT" << endl; break;
        case MiniFigDirection::BACK: cout << "Direction: BACK" << endl; break;
        case MiniFigDirection::LEFT: cout << "Direction: LEFT" << endl; break;
        case MiniFigDirection::RIGHT: cout << "Direction: RIGHT" << endl; break;
    }
}
...
void MiniFigActionHandler::execute(int position, CameraServer::MiniFigActionResponse& response)
{
    ...
    if(position == 0) {
        ...
        if(this->firstAttemptResult.wasDetected && this->firstAttemptResult.direction == MiniFigDirection::FRONT) {
        ...
        cout << "Subsequent attempt (position " << position << ") after failed detection. Saving for debug." << endl;
