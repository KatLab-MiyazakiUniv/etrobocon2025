...
MockResponse SpikeClient::handleCommand(spike::CommandId id, const std::vector<char>& request_payload) {
    if (commandHandlers_.count(id)) {
        return commandHandlers_[id](request_payload);
    } else {
        std::cerr << "Error: No handler registered for CommandId: " << static_cast<uint16_t>(id) << std::endl;
        return {false, {}};
    }
}
