...
void SocketClient::disconnectFromServer() {
    if (isConnected) {
        send(sock, reinterpret_cast<const char*>(&CameraServer::Command::SHUTDOWN), sizeof(CameraServer::Command), 0);
        close(sock);
        sock = -1;
        isConnected = false;
...
bool SocketClient::executeMiniFigAction(const CameraServer::MiniFigActionRequest& request, CameraServer::MiniFigActionResponse& response) {
    if (!isConnected) {
...
bool SocketClient::executeBackgroundPlaAction(const CameraServer::BackgroundPlaActionRequest& request, CameraServer::BackgroundPlaActionResponse& response) {
    if (!isConnected) {
        std::cerr << "Not connected to server." << std::endl;
        return false;
    }
