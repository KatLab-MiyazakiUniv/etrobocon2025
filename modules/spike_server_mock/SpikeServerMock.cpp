...
/**
 * @file   SpikeServerMock.cpp
 * @brief  SPIKEサーバーのモッククラスの実装
 * @author takuchi17
 */
...
spike::HsvResponse SpikeServerMock::hsv_value = { 180, 50, 80 };  // Default cyan-ish
bool SpikeServerMock::button_pressed_state[3] = { false, false, false };
bool SpikeServerMock::force_sensor_pressed_state = false;
float SpikeServerMock::force_sensor_value = 0.0f;
  ...
  while(true) {
    ...
    switch(request.id) {
      ...
      case spike::CommandId::MOTOR_SET_POWER: {
        ...
        double speed_to_set = 0.0;
        if(req.power > 0) {
          speed_to_set = 100.0;  // Arbitrary positive speed
        } else if(req.power < 0) {
          speed_to_set = -100.0;  // Arbitrary negative speed
        }
        if(req.target == spike::MotorTarget::RIGHT) {
          SpikeServerMock::right_motor_power = req.power;
          SpikeServerMock::right_motor_speed = speed_to_set;
        } else if(req.target == spike::MotorTarget::LEFT) {
          SpikeServerMock::left_motor_power = req.power;
          SpikeServerMock::left_motor_speed = speed_to_set;
      ...
      case spike::CommandId::MOTOR_SET_SPEED: {
        ...
        if(req.target == spike::MotorTarget::RIGHT) {
          ...
          SpikeServerMock::right_motor_power = power_to_set;  // Set power based on speed
        } else if(req.target == spike::MotorTarget::LEFT) {
          SpikeServerMock::left_motor_speed = req.speed;
          SpikeServerMock::left_motor_power = power_to_set;  // Set power based on speed
        }
        if(!sendData(client, &response_header, sizeof(response_header))) break;
        break;
      }
      ...
      case spike::CommandId::MOTOR_GET_COUNT: {
        ...
        int32_t increment = 0;
        if(req.target == spike::MotorTarget::RIGHT) {
          if(SpikeServerMock::right_motor_power >= 0)
            increment = 1000;
          else if(SpikeServerMock::right_motor_power < 0)
            increment = -1000;
          SpikeServerMock::right_motor_count += increment;
          res.value = SpikeServerMock::right_motor_count;
        } else if(req.target == spike::MotorTarget::LEFT) {
          if(SpikeServerMock::left_motor_power >= 0)
            increment = 1000;
          else if(SpikeServerMock::left_motor_power < 0)
            increment = -1000;
          SpikeServerMock::left_motor_count += increment;
          res.value = SpikeServerMock::left_motor_count;
        } else if(req.target == spike::MotorTarget::ARM) {
          if(SpikeServerMock::arm_motor_power >= 0)
            increment = 1000;
          else if(SpikeServerMock::arm_motor_power < 0)
            increment = -1000;
          SpikeServerMock::arm_motor_count += increment;
          res.value = SpikeServerMock::arm_motor_count;
        } else {
      ...
      case spike::CommandId::MOTOR_GET_SPEED: {
        ...
        spike::Int32Response res;
        if(req.target == spike::MotorTarget::RIGHT)
          res.value = static_cast<int32_t>(SpikeServerMock::right_motor_speed);
        else if(req.target == spike::MotorTarget::LEFT)
          res.value = static_cast<int32_t>(SpikeServerMock::left_motor_speed);
        else
          res.value = 0;  // Should not happen
      ...
      case spike::CommandId::COLOR_SENSOR_GET_COLOR_HSV: {
        ...
        SpikeServerMock::hsv_value.h = static_cast<uint16_t>(hsv_h_distribution(generator));
        SpikeServerMock::hsv_value.s = static_cast<uint8_t>(hsv_sv_distribution(generator) * 100);
        SpikeServerMock::hsv_value.v = static_cast<uint8_t>(hsv_sv_distribution(generator) * 100);
        spike::HsvResponse res = SpikeServerMock::hsv_value;
        std::cout << "MockServer: COLOR_SENSOR_GET_COLOR_HSV - H: " << res.h << ", S: " << res.s
                  << ", V: " << res.v << std::endl;
        if(!sendData(client, &res, sizeof(res))) break;
        break;
      }
      ...
      case spike::CommandId::CLOCK_SLEEP: {
        ...
        if(!receiveData(client, &req, sizeof(req))) break;
        std::cout << "MockServer: CLOCK_SLEEP - Microseconds: " << req.microseconds << std::endl;
        std::this_thread::sleep_for(std::chrono::microseconds(req.microseconds));
        if(!sendData(client, &response_header, sizeof(response_header))) break;
        break;
      }
      ...
      case spike::CommandId::FORCE_SENSOR_GET_FORCE: {
        ...
        spike::FloatResponse res{ SpikeServerMock::force_sensor_value };
