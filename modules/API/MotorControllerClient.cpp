...
{
  spikeClient.executeCommand(spike::CommandId::MOTOR_STOP_WHEELS);
}

// ブレーキをかけてタイヤのモータを停止する
void MotorControllerClient::brakeWheelsMotor()
{
  spikeClient.executeCommand(spike::CommandId::MOTOR_BRAKE_WHEELS);
}
...
{
  spikeClient.executeCommand(spike::CommandId::MOTOR_STOP_ARM);
}

// アームモータを止めて角度を維持する
void MotorControllerClient::holdArmMotor()
{
  spikeClient.executeCommand(spike::CommandId::MOTOR_HOLD_ARM);
}
...
{
  auto res = spikeClient.executeQuery<int32_t>(spike::CommandId::MOTOR_GET_POWER,
                                               spike::MotorTarget::RIGHT);
  return res.value_or(0);
}
