/**
 * @file   DualColorCameraLineTrace.cpp
...
DualColorCameraLineTrace::DualColorCameraLineTrace(
    Robot& _robot, COLOR _targetColor, double _targetDistance, double _targetSpeed,
    int _targetXCoordinate, int _targetXCoordinate2, const PidGain& _pidGain,
    std::unique_ptr<DualLineBoundingBoxDetector> _detector)
  : DualCameraPidTracking(_robot, _targetSpeed, _targetXCoordinate, _targetXCoordinate2, _pidGain,
                          *_detector),
...
bool DualColorCameraLineTrace::isMetPreCondition()
{
  // 目標の色がNoneのとき終了する
...
void DualColorCameraLineTrace::prepare()
{
  // 初期値を代入
...
bool DualColorCameraLineTrace::isMetContinuationCondition()
{
  // HSV値を取得
