...
void SmartCarryAction::run()
{
  ...
  UltrasonicDistanceCameraLineTrace udcl(robot, 70.0, ultrasonicDistance, 400, 400,
  ...
  std::cout << "UDCL終了時点の走行距離: " << currentDistance << std::endl;
  // DSで進むべき距離を計算する。(超音波距離 - 走行距離)
  double runDistance = fabs(currentDistance - initialDistance);
  std::cout << "UDCLの走行距離: " << runDistance << std::endl;
  double distanceToMove = ultrasonicDistance - runDistance;
  std::cout << "DSで進むべき距離: " << distanceToMove << std::endl;

  if(distanceToMove < 10.0) {
    distanceToMove = 0.0;  // 誤差と負の値をはじく
  }
